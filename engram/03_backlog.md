...
| GROW-16 | **Stickers** | trackeado (pendiente decisión PO) | catálogo para pegar sobre la foto. **Análisis monetización (2026-07-20):** el PO propuso premium con código rotativo por comprador → DESCARTADO: para mandar códigos nuevos hace falta servidor+DB+emails (rompe la tesis client-side), castiga al que pagó, y un código que valida en el navegador es extraíble/compartible (rotarlo no protege). Premium implica cuentas O license-keys de un procesador (Lemon Squeezy/Gumroad). **Recomendación: stickers GRATIS ahora** (set propio CC0 + subir los tuyos + emojis con set abierto tipo OpenMoji), premium a revisar DESPUÉS con tráfico real vía procesador de pago. No es prioridad (el problema es el tráfico). |
