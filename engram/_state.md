...
**🟡 F4 PARCIAL (e29a876):** páginas "alternativa a" **remove.bg** y **Photoroom**, ES+EN (4 págs), hreflang recíproco, enlazadas desde ambos hubs. **Sitemap 63→67. 0 huérfanas. Tests 137/137.** Datos de competidores verificados en fuente primaria y fechados (julio 2026); lo no confirmable va atribuido, no afirmado. Cada página dice en qué casos conviene el competidor (credibilidad + evita comparativa engañosa).
**⛔ F4 — photocut.ai NO se publicó (decisión pendiente del PO):** el supuesto del backlog era FALSO (ellos anuncian "no signup required" → nuestra cuña "sin registro" no diferencia contra ellos; la única válida es local-vs-nube) + colisión de marca (PhotoCut Studio vs PhotoCut). Detalle y recomendación en 03_backlog.md § "Nota GROW-8".
